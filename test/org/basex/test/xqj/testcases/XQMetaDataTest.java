...
public class XQMetaDataTest extends XQJTestCase {
  ...
  public void testGetProductMajorVersion() throws XQException {
    ...
    try {
      ...
      fail("A-XQMD-1.2: XQMetaData method failed with message: "
          + e.getMessage());
    }
  }
  ...
  public void testIsXQueryXSupported() throws XQException {
    ...
    if(supportsXQueryX) {
      try {
        String xqueryx = "<xqx:module xmlns:xqx='http://www.w3.org/2005/XQueryX'>"
            + "  <xqx:mainModule>"
            + "    <xqx:queryBody>"
        ...
        xqe.close();
        assertEquals(
            "XQueryX is supported, executing an XQueryX query yields unexpected results.",
            "Hello world!", s);
      } catch(XQException e) {
        fail("A-XQMD-2.1: XQueryX is supported, executing an XQueryX query failed with message: "
            + e.getMessage());
      }
    }
  }
  ...
  public void testIsTransactionSupported() throws XQException {
    ...
    if(supportsTransaction) {
      try {
        ...
        fail("A-XQMD-3.1: Transactions are supported, starting and ending a transaction failed with message: "
            + e.getMessage());
      }
    }
  }
  ...
  public void testIsFullAxisFeatureSupported() throws XQException {
    ...
    if(supportsFullAxis) {
      try {
        String xquery = "let $e := <root><e1/><e2/></root>" + "return"
        ...
        xqe.close();
        assertEquals(
            "Full Axis feature is supported, executing a query with full axis yields unexpected results.",
            "1", count);
      } catch(XQException e) {
        fail("A-XQMD-7.1: Full Axis feature is supported, executing a query with full axis failed with message: "
            + e.getMessage());
      }
    }
  }
  ...
  public void testWasCreatedFromJDBCConnection() throws XQException {
    ...
    try {
      ...
      assertFalse(
          "A-XQMD-11.1: XQMetaData reports that the connection was not created from a JDBC Connection.",
          wasFromJDBC);
    } catch(XQException e) {
      fail("A-XQMD-1.2: XQMetaData method failed with message: "
          + e.getMessage());
