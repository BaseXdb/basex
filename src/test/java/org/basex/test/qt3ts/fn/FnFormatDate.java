...
public class FnFormatDate extends QT3TestSet {
  ...
  public void formatDate011() {
    final XQuery query = new XQuery(
      ...
      "        return concat(\"[\", $d, \":\", format-date($d, '[w]', (), 'ISO', ()), ']')",
      ctx);
    query.bind("t", new XQuery("xs:date('2005-12-01')", ctx).value());
  ...
  /**
   * test format-date: fallback to English for unsupported language. Test assumes (a) that Iberian (xib) is not supported, and (b) that the fallback language is English. Changed from lang="fo" to lang="xib": See bug 862..
   */
  @org.junit.Test
  public void formatDateEn151() {
    final XQuery query = new XQuery(
