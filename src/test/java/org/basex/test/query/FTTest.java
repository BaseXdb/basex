...
public final class FTTest extends QueryTest {
  ...
  static {
    ...
    queries = new Object[][] {
      ...
      { "Simple 4", nod(22), "//b['true' contains text 'true']" },
      { "Simple 5", bool(true), "//@key contains text 'value'" },
      { "Simple 6", nod(36), "//@key[. contains text 'value']" },
      ...
      { "FT 1", nod(14), "//w[text() contains text 'HELLO']" },
      { "FT 2", nod(14), "//w[text() <- 'hello']" },
      { "FT 3", nod(14), "//w[text() <- '    hello!...   ']" },
      { "FT 4", empty(), "//w[  text  (   )  <-  '  crap  '  ]  " },
      { "FT 5", empty(), "//w[text() <- 'db']" },
      { "FT 6", nod(42, 46), "//mix[text() <- 'A']" },
      { "FT 7", nod(14), "//w[text() <- 'hello']['A' <- 'A']" },
      { "FT 8", nod(14), "//w[text() <- 'hello']['X' <- 'X' using fuzzy]" },
      { "FT 9", nod(14), "//w[text() = 'hello' and 'X' <- 'X']" },
      { "FT 10", nod(14), "//w[text() = 'hello' and text() <- 'hello']" },
      { "FT 11", empty(), "//wld[text() <- '']" },
      { "FT 12", empty(), "//wld[text() <- ' ']" },
      { "FT 13", nod(40), "//*[text() <- 'yeah']" },
      ...
      { "FTWildCard 13", bool(false), "'a' <- 'a.+' using wildcards" },
      { "FTWildCard 14", bool(true), "'aa' <- 'a.+' using wildcards" },
      { "FTWildCard 15", bool(true), "'aaaa' <- 'a.+' using wildcards" },
      { "FTWildCard 16", bool(true), "'a' <- 'a.*' using wildcards" },
      { "FTWildCard 17", bool(true), "'aaaaaa' <- 'a.*' using wildcards" },
      { "FTWildCard 18", bool(false), "'a' <- 'a.' using wildcards" },
      { "FTWildCard 19", bool(true), "'aa' <- 'a.' using wildcards" },
      { "FTWildCard 20", bool(true), "'a' <- 'a.{0,1}' using wildcards" },
      { "FTWildCard 21", bool(true), "'aa' <- 'a.{0,1}' using wildcards" },
      { "FTWildCard 22", "'aa' <- 'a.{0, 1}' using wildcards" },
      { "FTWildCard 23", "'a' <- 'a.{-1,1}' using wildcards" },
      { "FTWildCard 24", "'a' <- 'a.{1}' using wildcards" },
      { "FTWildCard 25", "'a' <- 'a.{1-5}' using wildcards" },

      { "FTFuzzy 1", nod(7, 9, 11), "//* [text() <- 'Database' using fuzzy]" },
      { "FTFuzzy 2", nod(7, 9, 11), "//* [text() <- 'Databaze' using fuzzy]" },
