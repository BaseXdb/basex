package org.basex.test.query;

import org.basex.core.AProp;
import org.basex.core.BaseXException;
import org.basex.core.Prop;
import org.junit.Test;

/**
 * Full-text test queries.
 *
 * @author Workgroup DBIS, University of Konstanz 2005-10, ISC License
 * @author Christian Gruen
 */
public final class FTTest extends QueryTest {
  /** Test all flag. */
  private static final boolean ALL = true;

  /** Constructor. */
  static {
    doc =
      "<fttest>\n" +
      "  <co>\n" +
      "     <w>xml in the first sentence. second sentence. " +
      "third sentence. fourth sentence. fifth sentence.</w>\n" +
      "     <w>XML xml XmL</w>\n" +
      "     <w>we have xml databases</w>\n" +
      "     <w>XML DATABASES</w>\n" +
      "     <w>XML &amp; Databases</w>\n" +
      "  </co>\n" +
      "  <wc>\n" +
      "     <w>hello</w>\n" +
      "  </wc>\n" +
      "  <sc>\n" +
      "     <s>di\u00e4t-joghurt</s>\n" +
      "     <s>diat-joghurt</s>\n" +
      "  </sc>\n" +
      "  <at><b>B</b>ad one</at>\n" +
      "  <fti>adfas wordt. ook wel eens</fti>" +
      "  <fti>wordt ook wel een s</fti>" +
      "  <fti>adfad. wordt\nook wel.eens a</fti>" +
      "  <fti>adfad wordt. ook\nwel een s adf</fti>" +
      "  <fti>adfad wordt ook. wel een s</fti>" +
      "  <atr key='value'/>" +
      "  <w>the fifth sentence. fourth sentence. " +
      "third sentence. second sentence. first sentence.</w>\n" +
      "  <wld/>\n" +
      "  <wld>yeah</wld>\n" +
      "  <mix>A<sub/>B</mix>\n" +
      "  <mix>B<sub/>A</mix>\n" +
      "</fttest>";

    queries = new Object[][] {
      { "Simple 1", bool(true),
        "'a' contains text 'a'" },
      { "Simple 2", bool(true),
        "'a b' contains text 'b'" },
      { "Simple 3", bool(false),
        "'abc' contains text 'b'" },
      { "Simple 4", nod(22),
        "//b['true' contains text 'true']" },
      { "Simple 5", bool(true),
        "//@key contains text 'value'" },
      { "Simple 6", nod(36),
        "//@key[. contains text 'value']" },
      { "Simple 7", bool(false),
        "//@key contains text 'values'" },
      { "Simple 8", bool(true),
        "number('100') + 23 contains text '123'" },
      { "Simple 9", bool(true),
        "true() contains text 'true'" },
      { "Simple 10", bool(true),
        "false() contains text 'false'" },
      { "Simple 11", bool(false),
        "'text' contains text ''" },

      { "FT 1", nod(14),
        "//w[text() contains text 'HELLO']" },
      { "FT 2", nod(14),
        "//w[text() contains text 'hello']" },
      { "FT 3", nod(14),
        "//w[text() contains text '    hello!...   ']" },
      { "FT 4", empty(),
        "//w[  text  (   )  contains text  '  crap  '  ]  " },
      { "FT 5", empty(),
        "//w[text() contains text 'db']" },
      { "FT 6", nod(42, 46),
        "//mix[text() contains text 'A']" },
      { "FT 7", nod(14),
        "//w[text() contains text 'hello']['A' contains text 'A']" },
      { "FT 8", nod(14),
        "//w[text() contains text 'hello']" +
        "['X' contains text 'X' using fuzzy]" },
      { "FT 9", nod(14),
        "//w[text() = 'hello' and 'X' contains text 'X']" },
      { "FT 10", nod(14),
        "//w[text() = 'hello' and text() contains text 'hello']" },
      { "FT 11", empty(),
        "//wld[text() contains text '']" },
      { "FT 12", empty(),
        "//wld[text() contains text ' ']" },
      { "FT 13", nod(40),
        "//*[text() contains text 'yeah']" },

      { "Preds 1", nod(7, 9, 11),
        "//w[text() contains text 'xml'][text() contains text 'Databases']" },
      { "Preds 2", nod(35),
        "//fttest[co/w contains text 'xml'][w contains text 'fifth']/atr" },
      { "Preds 3", nod(1),
        "//fttest[*/text() contains text 'ook']" +
        "[*/text() contains text 'een']" },
      { "Preds 4", nod(1),
        "*[*/text() contains text 'ook'][*/text() contains text 'een']" },
      { "Preds 5", nod(7),
        "//*[text() contains text 'have'][text() contains text 'xml']" },
      { "Preds 6", nod(13),
        "//*[*/text() contains text 'hello'][*/text() = 'hello']" },
      { "Preds 7", nod(7),
        "//w[text()[. contains text 'have xml'] contains text 'Databases']" },
      { "Preds 8", nod(5),
        "/descendant::w[text() contains text 'xml'][2]" },
      { "Preds 9", nod(3),
        "//w[text() contains text 'xml'][1]" },
      { "Preds 10", nod(5),
        "//w[text() contains text 'xml'][2]" },
      { "Preds 11", nod(14),
        "//wc/w[text() contains text 'hello']" },
      { "Preds 12", nod(46),
        "//mix[text()[1] contains text 'B']" },

      { "AndOr 1", nod(7, 9, 11),
        "//w[text() contains text 'xml' and " +
        "text() contains text 'databases']" },
      { "AndOr 2", nod(2),
        "//*[*/text() contains text 'have' and " +
        "*/text() contains text 'first']" },
      { "AndOr 3", nod(25, 29),
        "//fti[text() contains text 'eens' or text() contains text 'a']" },
      { "AndOr 4", nod(25, 29),
        "//fti[text() contains text 'eens' and text() contains text 'ook' or " +
        "text() contains text 'a']" },
      { "AndOr 5", nod(31),
        "//fti[text() contains text 'adf s' or text() contains text 's adf']" },
      { "AndOr 6", nod(31),
        "//fti[contains(text(), 'adf') and text() contains text 'adf']" },
      { "AndOr 7", nod(3),
        "//*[text() contains text 'sentence' and text() contains text 'xml']" },
      { "AndOr 8", nod(42, 46),
        "//mix[text() contains text 'A'][text() contains text 'B']" },

      { "Phrase 1", nod(7, 9, 11),
        "//w [text() contains text 'xml databases']" },
      { "Phrase 2", nod(7, 9, 11),
        "//w [text() contains text 'xml &amp; databases']" },
      { "Phrase 3", nod(7, 9, 11),
        "//w [text() contains text 'xml :) databases :|']" },
      { "Phrase 4", empty(),
        "//w [text() contains text 'xml db']" },
      { "Phrase 5", nod(25, 29),
        "/fttest/fti [text() contains text 'wordt ook wel eens']" },

      { "FTDiacritics 1", nod(17, 19),
        "//s [text() contains text 'diat']" },
      { "FTDiacritics 2", nod(17, 19),
        "//s [text() contains text 'di\u00e4t joghurt' using diacritics " +
        "insensitive]" },
      { "FTDiacritics 3", nod(17),
        "//s [text() contains text 'di\u00e4t joghurt' " +
        "using diacritics sensitive]" },

      { "FTCaseOption 1", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml']" },
      { "FTCaseOption 2", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' using case insensitive]" },
      { "FTCaseOption 3", nod(11),
        "/fttest/co/w [text() contains text 'XML Databases' " +
        "using case sensitive]" },
      { "FTCaseOption 4", nod(9),
        "/fttest/co/w [text() contains text 'xml databases' using uppercase]" },
      { "FTCaseOption 5", nod(3, 5, 7),
        "/fttest/co/w [text() contains text 'XML' using lowercase]" },
      { "FTCaseOption 6", nod(5, 9, 11),
        "/fttest/co/w [text() contains text 'xml' using uppercase]" },
      { "FTCaseOption 7", nod(7),
        "/fttest/co/w [text() contains text 'XML DATABASES' using lowercase]" },

      { "FTWildCard 1", nod(14),
        "/fttest/wc/w [text() contains text '.ello' using wildcards]" },
      { "FTWildCard 2", nod(14),
        "/fttest/wc/w [text() contains text 'hell.' using wildcards]" },
      { "FTWildCard 3", nod(14),
        "/fttest/wc/w [text() contains text '.+llo' using wildcards]" },
      { "FTWildCard 4", nod(14),
        "/fttest/wc/w [text() contains text 'hell.+' using wildcards]" },
      { "FTWildCard 5", nod(14),
        "/fttest/wc/w [text() contains text '.*llo' using wildcards]" },
      { "FTWildCard 6", nod(14),
        "/fttest/wc/w [text() contains text 'hel.*' using wildcards]" },
      { "FTWildCard 7", nod(14),
        "/fttest/wc/w [text() contains text '.*' using wildcards]" },
      { "FTWildCard 8", nod(14),
        "/fttest/wc/w [text() contains text '.+' using wildcards]" },
      { "FTWildCard 9", nod(5, 9, 11),
        "/fttest/co/w [text() contains text 'X.+' " +
        "using wildcards using case sensitive]" },
      { "FTWildCard 10", nod(5, 9, 11),
        "/fttest/co/w [text() contains text 'x.+' using wildcards " +
        "using uppercase]" },
      { "FTWildCard 11", nod(40),
        "/fttest/wld [text() contains text '.*' using wildcards]" },
      { "FTWildCard 12", nod(40),
        "/fttest/wld [text() contains text '.+' using wildcards]" },
      { "FTWildCard 13", nod(14),
        "//w [text() contains text 'he.{1,2}o' using wildcards]" },
      { "FTWildCard 14", nod(14),
        "//w [text() contains text 'h.+ll.+' using wildcards]" },
      { "FTWildCard 15", nod(14),
        "//w [text() contains text 'h.\\llo' using wildcards]" },

      { "FTWildCard 13", bool(false),
        "'a' contains text 'a.+' using wildcards" },
      { "FTWildCard 14", bool(true),
        "'aa' contains text 'a.+' using wildcards" },
      { "FTWildCard 15", bool(true),
        "'aaaa' contains text 'a.+' using wildcards" },
      { "FTWildCard 16", bool(true),
        "'a' contains text 'a.*' using wildcards" },
      { "FTWildCard 17", bool(true),
        "'aaaaaa' contains text 'a.*' using wildcards" },
      { "FTWildCard 18", bool(false),
        "'a' contains text 'a.' using wildcards" },
      { "FTWildCard 19", bool(true),
        "'aa' contains text 'a.' using wildcards" },
      { "FTWildCard 20", bool(true),
        "'a' contains text 'a.{0,1}' using wildcards" },
      { "FTWildCard 21", bool(true),
        "'aa' contains text 'a.{0,1}' using wildcards" },
      { "FTWildCard 22", // error (space)
        "'aa' contains text 'a.{0, 1}' using wildcards" },
      { "FTWildCard 23", // error (negative number)
        "'a' contains text 'a.{-1,1}' using wildcards" },
      { "FTWildCard 24", // error (missing second value)
        "'a' contains text 'a.{1}' using wildcards" },
      { "FTWildCard 25", // error (wrong syntax)
        "'a' contains text 'a.{1-5}' using wildcards" },

      { "FTFuzzy 1", nod(7, 9, 11),
        "//* [text() contains text 'Database' using fuzzy]" },
      { "FTFuzzy 2", nod(7, 9, 11),
        "//* [text() contains text 'Databaze' using fuzzy]" },
      { "FTFuzzy 3", empty(),
        "//* [text() contains text 'Databasing' using fuzzy]" },

      { "FTAnyAllOption 1", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' any]" },
      { "FTAnyAllOption 2", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' all]" },
      { "FTAnyAllOption 3", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' any word]" },
      { "FTAnyAllOption 4", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' all words]" },
      { "FTAnyAllOption 5", nod(3, 5, 7, 9, 11),
        "/fttest/co/w [text() contains text 'xml' phrase]" },
      { "FTAnyAllOption 6", bool(false),
        "'text' contains text { '' } any" },
      { "FTAnyAllOption 7", bool(false),
        "'text' contains text { '' } all" },
      { "FTAnyAllOption 8", bool(false),
        "'text' contains text { '' } all words" },
      { "FTAnyAllOption 9", bool(false),
        "'text' contains text { '' } any word" },
      { "FTAnyAllOption 10", bool(false),
        "'text' contains text { '' } phrase" },
      { "FTAnyAllOption 11", bool(true),
        "'red balloon' contains text { 'red', '', 'balloon' } any" },
      { "FTAnyAllOption 12", bool(false),
        "'red balloon' contains text { 'red', '', 'balloon' } all" },
      { "FTAnyAllOption 13", bool(true),
        "'red balloon' contains text { 'red', '', 'balloon' } all words" },
      { "FTAnyAllOption 14", bool(true),
        "'red balloon' contains text { 'red', '', 'balloon' } any word" },
      { "FTAnyAllOption 15", bool(true),
        "'red balloon' contains text { 'red', '', 'balloon' } phrase" },

      { "FTTimes 1", nod(7, 9, 11),
        "//w [text() contains text 'DaTaBaSeS' occurs exactly 1 times]" },
      { "FTTimes 2", nod(5),
        "//w [text() contains text 'XmL' occurs exactly 3 times]" },
      { "FTTimes 3", nod(14, 37),
        "//w [text() contains text 'xml' occurs exactly 0 times]" },
      { "FTTimes 4", nod(5),
        "//w [text() contains text 'xml' occurs at least 3 times]" },
      { "FTTimes 5", nod(3, 7, 9, 11, 14, 37),
        "//w [text() contains text 'XmL' occurs at most 2 times]" },
      { "FTTimes 6", nod(3, 7, 9, 11, 14, 37),
        "//w [text() contains text 'XmL' occurs from 0 to 1 times]" },
      { "FTTimes 7", nod(5),
        "//w [text() contains text 'xml xml' occurs at least 2 times]" },
      { "FTTimes 8", empty(),
        "//w [text() contains text 'xml xml' occurs at least 4 times]" },

      { "FTAndOr 1", nod(7, 9, 11),
        "//w [text() contains text 'XmL' ftand 'Databases']" },
      { "FTAndOr 2", nod(7, 9, 11, 14),
        "//w [text() contains text 'databases' ftor 'hello']" },
      { "FTAndOr 3", nod(14),
        "//w [text() contains text 'HELLO' ftand 'hello']" },
      { "FTAndOr 4", nod(7, 9, 11, 14),
        "//w [text() contains text 'xml' ftand 'databases' ftor 'hello' ]" },
      { "FTAndOr 5", nod(7, 9, 11),
        "//w [text() contains text 'databases' ftand ('xml' ftor 'hello')]" },
      { "FTAndOr 6", nod(31, 33),
        "//fti [text() contains text 'adfad' ftand 'wordt' ftand 'ook' " +
        "ftand 'wel' ftand 'een' ftand 's']" },
      { "FTAndOr 7", empty(),
        "//* [text() contains text 'databases' ftand 'db']" },
      { "FTAndOr 8", nod(14),
        "//* [text() contains text 'hola' ftor 'hello']" },
      { "FTAndOr 9", empty(),
        "//* [text() contains text 'hola' ftand 'hello']" },
      { "FTAndOr 10", nod(14),
        "//w [text() contains text 'HELLO' ftand ('hello' using stemming)]" },
      { "FTAndOr 11", nod(14),
        "//w [text() contains text 'HELLO' ftand ('hello') using stemming]" },

      { "FTStemming 1", nod(7, 9, 11),
        "//w [text() contains text 'xml database' using stemming]" },
      { "FTStemming 2", empty(),
        "//w [text() contains text 'xml database' using no stemming]" },
      { "FTStemming 3", nod(7, 9, 11),
        "//w [text() contains text 'xml' ftand 'databasing' using stemming]" },
      { "FTStemming 4", nod(7, 9, 11, 14),
        "//w [text() contains text 'hello' ftor 'database' using stemming]" },
      { "FTStemming 5", nod(3, 5, 14, 37),
        "//w [text() contains text ftnot 'database' using stemming]" },
      { "FTStemming 6", bool(true),
        "'base' contains text 'bases' using stemming" },
      { "FTStemming 7", bool(true),
        "'bases' contains text ('base') using stemming" },
      { "FTStemming 8", bool(true),
        "'base' contains text ('bases') using stemming" },
      { "FTStemming 9", bool(true),
        "'base' contains text ('bases' using stemming) using no stemming" },

      { "FTLanguage 1", nod(14),
        "//* [text() contains text 'hello' using language 'en']" },
      { "FTLanguage 2", nod(14),
        "//* [text() contains text 'hello' using language 'de']" },
      { "FTLanguage 3", // error
        "//* [text() contains text 'hello' using language 'jp']" },

      { "FTStopWords 1", nod(7, 9, 11), "//* [text() contains text " +
        "'and databases' using stop words ('xml', 'and')]" },
      { "FTStopWords 2", nod(7),
        "//* [text() contains text 'we use xml' using stop words ('use')]" },

      { "FTAtomization 1", nod(21),
        "//at [. contains text 'bad one']" },
      { "FTAtomization 2", nod(35),
        "//atr [@key contains text 'value']" },

      { "FTOrdered 1", nod(7, 9, 11),
        "//w [. contains text 'databases' ordered]" },
      { "FTOrdered 2", nod(7, 9, 11),
        "//w [. contains text 'xml' ftand 'databases' ordered]" },
      { "FTOrdered 3", empty(),
        "//w [. contains text 'databases' ftand 'xml' ordered]" },
      { "FTOrdered 4", bool(true),
        "'A B' contains text ('A' ftand 'B' ordered)" },
      { "FTOrdered 5", bool(true),
        "'A B' contains text ('A' ftand 'B') ftor ('C' ftand 'D') ordered" },
      { "FTOrdered 6", bool(true),
        "'C D' contains text ('A' ftand 'B') ftor ('C' ftand 'D') ordered" },
      { "FTOrdered 7", bool(true),
        "'A B C D' contains text ('B' ftand 'A' ordered) ftor " +
        "('C' ftand 'D' ordered) ordered" },
      { "FTOrdered 8", bool(false),
        "'B A' contains text ('A' ftand ftnot 'B') ordered" },
      { "FTOrdered 9", bool(true),
        "'A B' contains text 'B' ftor 'A' ordered" },
      { "FTOrdered 10", bool(true),
        "'A B' contains text ('B' ftor 'A') ordered" },
      { "FTOrdered 11", bool(true),
        "'A B C' contains text ('A' ftor 'C') ftand 'B' ordered" },

      { "FTDistance 1", nod(3),
        "//w [text() contains text 'the' ftand 'fourth' " +
        "distance exactly 2 sentences]" },
      { "FTDistance 2", nod(3, 37),
        "//w [. contains text 'first' ftand 'second' ftand 'third' " +
          "distance exactly 1 words]" },
      { "FTDistance 3", nod(3, 37),
        "//w [. contains text 'first sentence' ftand 'third sentence' " +
        "distance exactly 2 words]" },
      { "FTDistance 4", nod(3),
        "//w [. contains text 'the first sentence' ftand 'third sentence' " +
        "distance exactly 2 words]" },
      { "FTDistance 5", nod(3, 37),
        "//w [. contains text 'sentence' ftand 'the' " +
        "distance exactly 1 words]" },
      { "FTDistance 6", nod(3, 37),
        "//w [. contains text ('second' ftand 'third' window 3 words) " +
        "ftand 'sentence' distance exactly 0 words]" },
      { "FTDistance 7", nod(3, 37),
        "//w [text() contains text ('second' ftand 'third' window 3 words) " +
        "ftand 'sentence' distance exactly 0 words ordered]" },
      { "FTDistance 8", nod(3, 37),
        "//w [. contains text 'third' ftand 'second' " +
        " ftand 'first' distance exactly 1 words ordered]" },
      { "FTDistance 9", nod(3, 37),
        "//w [. contains text 'first' ftand 'second' " +
        " ftand 'third' distance exactly 1 words ordered]" },
      { "FTDistance 10", bool(true),
        "'a b' contains text 'a' ftand ('b') distance exactly 0 words" },
      { "FTDistance 11", bool(true),
        "'a b' contains text ('a') ftand ('b') entire content" },

      { "FTWindow 1", nod(3, 37),
        "//w [. contains text 'second' ftand 'fifth' window 7 words]" },
      { "FTWindow 2", nod(3, 37),
        "//w [. contains text 'second sentence' ftand 'fifth sentence' " +
        "window 8 words]" },
      { "FTWindow 3", nod(3, 37),
        "//w [. contains text 'third' ftand 'second' " +
        "ftand 'fifth' window 7 words]" },
      { "FTWindow 4", nod(37),
        "//w [. contains text 'fifth' ftand 'third' " +
        "ftand 'second' ordered window 7 words]" },
      { "FTWindow 4", nod(3, 37),
        "//w [. contains text 'fifth' ftand 'third' " +
        "ftand 'second' window 7 words ordered]" },

      { "FTScope 1", nod(25, 27, 29, 31, 33),
        "//fti [. contains text 'wordt ook' same sentence]" },
      { "FTScope 2", nod(27, 29, 33),
        "//fti [text() contains text 'wordt' ftand 'ook' same sentence]" },
      { "FTScope 3", nod(25, 31),
        "//fti [. contains text 'wordt' ftand 'ook' different sentence]" },
      { "FTScope 4", nod(25, 27, 29, 33),
        "//fti [. contains text 'ook' ftand 'wel' same paragraph]" },
      { "FTScope 5", nod(31),
        "//fti [. contains text 'ook' ftand 'wel' different paragraph]" },
      { "FTScope 6", bool(true),
        "'a. a b' contains text ('a' ftand 'b') different sentence" },

      { "FTContent 1", nod(3, 5, 9, 11),
        "//w [text() contains text 'xml' at start]" },
      { "FTContent 2", empty(),
        "//w [. contains text 'databases' at start]" },
      { "FTContent 3", nod(9, 11),
        "//w [. contains text 'xml databases' at start]" },
      { "FTContent 4", nod(9, 11),
        "//w [. contains text 'xml' ftand 'databases' ordered at start]" },
      { "FTContent 5", nod(7, 9, 11),
        "//w [. contains text 'databases' at end]" },
      { "FTContent 6", nod(7, 9, 11),
        "//w [. contains text 'xml databases' at end]" },
      { "FTContent 7", nod(7, 9, 11),
        "//w [. contains text 'xml' ftand 'databases' at end]" },
      { "FTContent 8", empty(),
        "//w [. contains text 'have xml' at end]" },
      { "FTContent 9", nod(14),
        "//w [text() contains text 'hello' entire content]" },
      { "FTContent 10", nod(9, 11),
        "//w [. contains text 'xml databases' entire content]" },
      { "FTContent 11", bool(true),
        "'a b c d' contains text 'a' ftand 'b' ftand 'c'" +
        " ftand 'd' entire content" },
      { "FTContent 12", bool(true),
        "'a b c d' contains text 'd' ftand 'c' ftand 'b'" +
        " ftand 'a' entire content" },
      { "FTContent 13", bool(true),
        "'a b c d' contains text 'a' ftand 'b' ftand 'c'" +
        " ftand 'd' entire content ordered" },
      { "FTContent 14", bool(false),
        "'a b c d' contains text 'd' ftand 'c' ftand 'b'" +
        " ftand 'a' entire content ordered" },
      { "FTContent 15", bool(true),
        "'a b c d' contains text 'a' ftand 'b' at start" },
      { "FTContent 16", bool(true),
        "'a b c d' contains text 'a' ftand 'b' at start ordered" },
      { "FTContent 17", bool(true),
        "'a b c d' contains text 'b' ftand 'a' at start" },
      { "FTContent 18", bool(false),
        "'a b c d' contains text 'b' ftand 'a' at start ordered" },
      { "FTContent 19", bool(true),
        "'a b c d' contains text 'c' ftand 'd' at end" },
      { "FTContent 20", bool(true),
        "'a b c d' contains text 'c' ftand 'd' at end ordered" },
      { "FTContent 21", bool(true),
        "'a b c d' contains text 'd' ftand 'c' at end" },
      { "FTContent 22", bool(false),
        "'a b c d' contains text 'd' ftand 'c' at end ordered" },
      { "FTContent 23", bool(true),
        "'a b c' contains text 'b c' ftand 'a' entire content" },
      { "FTContent 24", bool(true),
        "'a b c' contains text 'a' ftand 'b c' entire content" },
      { "FTContent 25", bool(true),
        "'a b c' contains text 'a b c' entire content" },
      { "FTContent 26", bool(false),
        "'a b' contains text 'a' entire content" },
      { "FTContent 27", bool(false),
        "'a b' contains text 'b' entire content" },

      { "FTMildNot 1", nod(3, 5),
        "//w [text() contains text 'xml' not in 'xml databases']" },
      { "FTMildNot 2", nod(14),
        "//w [text() contains text 'hello' not in 'xml']" },
      { "FTMildNot 3", bool(false),
        "'a b' contains text 'a' not in 'a b'" },
      { "FTMildNot 4", bool(false),
        "'a' contains text 'a' not in 'a'" },
      { "FTMildNot 5", bool(true),
        "'a b' contains text 'a b' not in 'a'" },
      { "FTMildNot 6", bool(true),
        "'a b a' contains text 'a' not in 'a b'" },
      { "FTMildNot 7", bool(false),
        "'a b a b' contains text 'a' not in 'a b'" },
      { "FTMildNot 8",
        "'a' contains text 'a' not in ftnot 'a'" },
      { "FTMildNot 9", nod(3, 5, 7, 9, 11),
        "//w [text() contains text 'xml' not in 'we have']" },

      { "FTUnaryNot 1", nod(14, 37),
        "//w [text() contains text ftnot 'xml']" },
      { "FTUnaryNot 2", nod(3, 5),
        "//w [text() contains text 'xml' ftand ftnot 'databases']" },
      { "FTUnaryNot 3", nod(3, 5),
        "//w [text() contains text ftnot 'databases' ftand 'xml']" },
      { "FTUnaryNot 4", nod(31),
        "//* [text() contains text 'adf' ftand ftnot 'xml']" },
      { "FTUnaryNot 5", nod(3, 5, 9, 11),
        "//w [text() contains text 'xml' ftand ftnot 'databases' " +
        "using case sensitive]" },
      { "FTUnaryNot 6", nod(7, 9, 11, 14, 37),
        "//w [text() contains text 'databases' ftor ftnot 'xml']" },
      { "FTUnaryNot 7", nod(3, 5, 14, 37),
        "//w [text() contains text 'hello' ftor ftnot 'databases']" },
      { "FTUnaryNot 8", nod(3, 5, 7, 9, 11, 14, 37),
        "//w [text() contains text ftnot 'bier']" },
      { "FTUnaryNot 9", nod(3, 5, 7, 9, 11, 14, 37),
        "//w [text() contains text ftnot 'bier' ftand ftnot 'wein' ]" },
      { "FTUnaryNot 10", nod(3, 5, 7, 9, 11, 14, 37),
        "//w [text() contains text ftnot 'bier' ftor ftnot 'wein' ]" },
      { "FTUnaryNot 11", nod(14, 37),
        "//w [text() contains text ftnot 'xml' ftand ftnot 'databeses' ]" },
      { "FTUnaryNot 12", nod(31),
        "//fti [text() contains text ftnot (ftnot 'adf') ]" },
      { "FTUnaryNot 13", nod(31),
        "//fti [text() contains text 'adf' ftand ftnot (ftnot 'adf')]" },
      { "FTUnaryNot 14", nod(31),
        "//fti [text() contains text 'adf' ftor ftnot (ftnot 'adf')]" },
      { "FTUnaryNot 15", nod(25, 27, 29, 31, 33),
        "//fti [text() contains text 'adf' ftor ftnot 'adf']" },
    };
  }

  @Test
  @Override
  public void test() throws BaseXException {
    final Prop prop = context.prop;
    if(ALL) {
      // testing all kinds of combinations
      for(int a = 0; a < 2; ++a) { prop.set(Prop.FTINDEX, a == 0);
        for(int b = 0; b < 2; ++b) { prop.set(Prop.WILDCARDS, b == 0);
          for(int c = 0; c < 2; ++c) { prop.set(Prop.STEMMING, c == 0);
            for(int d = 0; d < 2; ++d) { prop.set(Prop.DIACRITICS, d == 0);
              for(int e = 0; e < 2; ++e) { prop.set(Prop.CASESENS, e == 0);
                super.test();
              }
            }
          }
        }
      }
    } else {
      // single test
      //prop.set(Prop.MAINMEM, true);
      prop.set(Prop.FTINDEX, true);
      prop.set(Prop.WILDCARDS, true);
      prop.set(Prop.STEMMING, true);
      prop.set(Prop.DIACRITICS, true);
      prop.set(Prop.CASESENS, true);
      super.test();
    }
  }

  @Override
  protected String details() {
    final Prop prop = context.prop;
    final StringBuilder sb = new StringBuilder();
    sb.append(set(Prop.FTINDEX, prop) + ";");
    sb.append(set(Prop.WILDCARDS, prop) + ";");
    sb.append(set(Prop.STEMMING, prop) + ";");
    sb.append(set(Prop.DIACRITICS, prop) + ";");
    sb.append(set(Prop.CASESENS, prop));
    return sb.toString();
  }

  /**
   * Returns a flag string.
   * @param key key
   * @param prop properties
   * @return string
   */
  private String set(final Object[] key, final AProp prop) {
    return "set " + key[0] + " " + prop.is(key);
  }

  /* TABLE REPRESENTATION
  PRE DIS SIZ ATS  NS  KIND  CONTENT
    0   1  46   1   0  DOC   tmp
    1   1  45   1   0  ELEM  fttest
    2   1  11   1   0  ELEM  co
    3   1   2   1   0  ELEM  w
    4   1   1   1   0  TEXT  xml in the first sentence. second sentence.
      third sentence. fourth sentence. fifth sentence.
    5   3   2   1   0  ELEM  w
    6   1   1   1   0  TEXT  XML xml XmL
    7   5   2   1   0  ELEM  w
    8   1   1   1   0  TEXT  we have xml databases
    9   7   2   1   0  ELEM  w
   10   1   1   1   0  TEXT  XML DATABASES
   11   9   2   1   0  ELEM  w
   12   1   1   1   0  TEXT  XML & Databases
   13  12   3   1   0  ELEM  wc
   14   1   2   1   0  ELEM  w
   15   1   1   1   0  TEXT  hello
   16  15   5   1   0  ELEM  sc
   17   1   2   1   0  ELEM  s
   18   1   1   1   0  TEXT  di\u00e4t-joghurt
   19   3   2   1   0  ELEM  s
   20   1   1   1   0  TEXT  diat-joghurt
   21  20   4   1   0  ELEM  at
   22   1   2   1   0  ELEM  b
   23   1   1   1   0  TEXT  B
   24   3   1   1   0  TEXT  ad one
   25  24   2   1   0  ELEM  fti
   26   1   1   1   0  TEXT  adfas wordt. ook wel eens
   27  26   2   1   0  ELEM  fti
   28   1   1   1   0  TEXT  wordt ook wel een s
   29  28   2   1   0  ELEM  fti
   30   1   1   1   0  TEXT  adfad. wordt
  ook wel.eens a
   31  30   2   1   0  ELEM  fti
   32   1   1   1   0  TEXT  adfad wordt. ook
  wel een s adf
   33  32   2   1   0  ELEM  fti
   34   1   1   1   0  TEXT  adfad wordt ook. wel een s
   35  34   2   2   0  ELEM  atr
   36   1   1   1   0  ATTR  key="value"
   37  36   2   1   0  ELEM  w
   38   1   1   1   0  TEXT  the fifth sentence. fourth sentence.
     third sentence. second sentence. first sentence.
   39   1   1   1   0  ELEM  wld
   40   1   2   1   0  ELEM  wld
   41   1   1   1   0  TEXT  yeah
   42  41   4   1   0  ELEM  mix
   43   1   1   1   0  TEXT  A
   44   2   1   1   0  ELEM  sub
   45   3   1   1   0  TEXT  B
   */
}
