...
public final class XPathMarkFTTest extends QueryTest {
  ...
  static {
    ...
    queries = new Object[][] {
      { "A01", nod(59, 64, 79), "//L/*" },
      { "A02", nod(20), "//L/parent::*" },
      { "A03", nod(59, 64, 68, 73, 79), "//L/descendant::*" },
      { "A04", nod(53, 59, 64, 68, 73, 79), "//L/descendant-or-self::*" },
      { "A05", nod(1, 20), "//L/ancestor::*" },
      { "A06", nod(1, 20, 53), "//L/ancestor-or-self::*" },
      { "A07", nod(85, 99), "//L/following-sibling::*" },
      { "A08", nod(24, 39), "//L/preceding-sibling::*" },
      { "A09", nod(85, 89, 94, 99, 103, 108, 113, 117, 122),
        "//L/following::*" },
      { "A10", nod(6, 10, 15, 24, 28, 33, 39, 43, 48), "//L/preceding::*" },
      { "A11", nod(53), "//L/self::*" },
      { "A12", nod(54), "//L/@id" },
      { "F01", nod(1, 20, 53, 64, 73), "//*[contains(., 'plentiful')]" },
      { "F02", nod(73), "//*[starts-with(., 'plentiful')]" },
      { "F03", nod(73), "//*[substring(., 1, 9) = 'plentiful']" },
      { "F04", nod(68), "//*[substring-after(., 'oven') = 'ware']" },
      { "F05", nod(73), "//*[substring-before(., 'ful') = 'plenti']" },
      { "F06", nod(1, 20), "//*[string-length(translate(" +
        "normalize-space(.), ' ', '')) > 100]" },
      { "F07", nod(59), "//*[concat(., ..) = ..]" },
      { "F08", nod(1, 6, 20, 24, 39, 53, 85, 99, 113),
        "//*[ceiling(@pre div @post) = 1]" },
      { "F09", nod(1, 6, 20, 53, 113), "//*[floor(@pre div @post) = 0]" },
      { "F10", nod(1, 20), "//*[round(@pre div @post) = 0]" },
      { "F11", nod(113), "//*[name(.) = 'X']" },
      { "F12", nod(122), "//*[lang('it')]" },
      { "F13", nod(79), "//L/child::*[last()]" },
      { "F14", nod(73), "//L/descendant::*[4]" },
      { "F15", nod(1), "//L/ancestor::*[2]" },
      { "F16", nod(85), "//L/following-sibling::*[1]" },
      { "F17", nod(39), "//L/preceding-sibling::*[1]" },
      { "F18", nod(113), "//L/following::*[7]" },
      { "F19", nod(15), "//L/preceding::*[7]" },
      { "F20", nod(68, 73), "//*[count(ancestor::*) > 3]" },
      { "F21", nod(1, 6, 20, 24, 39, 53, 64, 85, 99, 113),
        "//*[sum(ancestor::*/@pre) < sum(descendant::*/@pre)]" },
      { "F22", nod(1, 122), "id('n1 n26')" },
      { "F23", nod(33, 79, 122),
        "id(id(//*[.='happy-go-lucky man']/@idrefs)/@idrefs)" },
      { "F24", nod(1, 6, 20, 53, 113), "//*[number(@pre) < number(@post)]" },
      { "F25", nod(1), "//*[string(@pre - 1) = '0']" },
      { "F26", nod(1, 6, 10, 15, 20, 24, 28, 39, 43, 48, 53, 59, 64, 68,
          73, 85, 89, 94, 99, 103, 108, 113, 117),
          "//*[boolean(@id) = true() and boolean(@idrefs) = false()]" },
      { "O01", nod(85, 99, 113), "//*[child::* and preceding::Q]" },
      { "O02", nod(89, 94, 103, 108, 117, 122),
          "//*[not(child::*) and preceding::Q]" },
      { "O03", nod(6, 10, 15, 24, 28, 33, 39, 43, 48, 85, 89, 94, 99,
          103, 108, 113, 117, 122), "//*[preceding::L or following::L]" },
      { "O04", nod(1, 20, 59, 64, 68, 73, 79),
          "//L/ancestor::* | //L/descendant::*" },
      { "O05", nod(33), "//*[.='happy-go-lucky man']" },
      { "O06", nod(59, 64, 68, 73, 79), "//*[@pre > 12 and @post < 15]" },
      { "O07", nod(1, 6, 10, 15, 20, 28, 33, 43, 48, 53, 59, 64, 68, 73,
          79, 89, 94, 103, 108, 113, 117, 122),
          "//*[@pre != @post]" },
      { "O08", nod(6, 24, 39, 48, 53, 59, 64, 79, 85, 89, 94, 99, 103,
          108, 113, 117, 122), "//*[((@post * @post + @pre * @pre) div " +
          "(@post + @pre)) > ((@post - @pre) * (@post - @pre))]" },
      { "O09", nod(6, 15, 24, 33, 43, 53, 64, 73, 85, 94, 103, 113, 122),
          "//*[@pre mod 2 = 0]" },
      { "P01", nod(20), "//*[L]" },
      { "P02", nod(59, 64, 79), "//*[parent::L]" },
      { "P03", nod(1, 20), "//*[descendant::L]" },
      { "P04", nod(1, 20, 53), "//*[descendant-or-self::L]" },
      { "P05", nod(59, 64, 68, 73, 79), "//*[ancestor::L]" },
      { "P06", nod(53, 59, 64, 68, 73, 79), "//*[ancestor-or-self::L]" },
      { "P07", nod(24, 39), "//*[following-sibling::L]" },
      { "P08", nod(85, 99), "//*[preceding-sibling::L]" },
      { "P09", nod(6, 10, 15, 24, 28, 33, 39, 43, 48), "//*[following::L]" },
      { "P10", nod(85, 89, 94, 99, 103, 108, 113, 117, 122),
        "//*[preceding::L]" },
      { "P11", nod(53), "//*[self::L]" },
      { "P12", nod(1, 6, 10, 15, 20, 24, 28, 33, 39, 43, 48, 53, 59, 64,
          68, 73, 79, 85, 89, 94, 99, 103, 108, 113, 117, 122),
          "//*[@id]" },
      { "T01", nod(58, 63), "//L/text()" },
      { "T02", nod(57), "//L/comment()" },
      { "T03", nod(78), "//L/processing-instruction()" },
      { "T04", nod(78), "//L/processing-instruction('myPI')" },
      { "T05", nod(57, 58, 59, 63, 64, 78, 79), "//L/node()" },
      { "T06", nod(64), "//L/N" },
      { "T07", nod(59, 64, 79), "//L/*" }
    };
  }
