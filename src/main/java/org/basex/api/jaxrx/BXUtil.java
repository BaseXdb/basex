...
public final class BXUtil {
  ...
  static void query(final String resource, final OutputStream out,
    ...
    final SerializeProp props = new SerializeProp(serialize);
    String log = props.log();
    if(wrap != null) {
      if(wrap.equals(SerializeProp.YES)) {
        ...
        log += Main.info(Text.SETVAL + Text.NL, EURLParameter.WRAP, wrap);
      }
    }
    if(log.length() != 0) badRequest(log.toString());
