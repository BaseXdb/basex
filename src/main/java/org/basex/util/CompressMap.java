 ...
 */
public final class CompressMap {
  /** Byte list. */
   ...
   */
  public CompressMap(final int m) {
    min = m;
  }
  ...
  public byte[] unpack(final byte[] txt) {
    ...
    for(int r = 0; r < l; r++) {
      ...
      if(pull() != 0) { // 1 xxx
        b = pull(3);
      } else if(pull() != 0) { // 01 xxx
        b = pull(3) | 0x08;
      } else if(pull() != 0) { // 001 xxxx
        b = pull(4) | 0x10;
      } else if(pull() != 0) { // 0001 xxxxx
        b = pull(5) | 0x20;
      } else { // 0000 xxxxxxxx
        b = pull(8);
      }
  ...
  /**
   * Pulls a single bit and returns the result.
   * @return result
   */
  private int pull() {
    final int x = (bl.list[c] & 1 << o) != 0 ? 1 : 0;
    if(++o == 8) {
      o = 0;
      c++;
    }
