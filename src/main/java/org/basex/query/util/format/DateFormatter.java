...
public final class DateFormatter {
  ...
  public static byte[] format(final Date date, final String pic,
      final byte[] lng, final byte[] cal, final byte[] plc)
    ...
    final DateParser fp = new DateParser(pic);
    while(fp.more()) {
      ...
      if(ch != 0) {
        ...
        String m = fp.marker();
        if(m.length() == 0) Err.or(PICDATE, pic);
        ...
        if(mp.error) Err.or(PICDATE, pic);
