  ...
  public Iter iter(final XQContext ctx) {
    return new FTNodeIter(){
      ...
      public FTNodeItem next() throws XQException { 
        return  FTMildNotIndex.this.next(ctx);
      }
    };
  }

  /**
   * Check if expr[0] has more results.
   * @param ctx current context
   * @return FTNodeItem next result
   * @throws XQException Exception
   */
  public boolean more(final XQContext ctx) throws XQException {
    n0 = (FTNodeItem) ctx.iter(expr[0]).next();
    return n0.ftn.size > 0;
  }
