...
public final class FTPos extends FTExpr {
  ...
  public boolean filter(final QueryContext ctx) throws QueryException {
    ...
    if(dunit != null) {
      ...
      if(!distance(mn, mx)) return false;
    }
    // ...window?
    if(wunit != null) {
  ...
  private boolean diff(final BoolList bl, final int n) {
    ...
    for(int i = 0; i < pos[n].size; i++) {
      final int p = calcPosition(pos[n].list[i], sdunit);
      if(p < bl.size && bl.get(p)) continue;
      bl.set(true, p);
  ...
  private int calcPosition(final int p, final FTUnit u) {
    ...
    return u == FTUnit.SEN ? iter.sent : iter.para;
  }
  ...
  public void indexAccessible(final QueryContext ctx, final IndexContext ic) 
    ...
    ic.iu &= !ic.ftnot || window == null && dist == null && !ordered &&
      !start && !end && !content && !same && !different;
  }
