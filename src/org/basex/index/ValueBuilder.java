...
public final class ValueBuilder extends IndexBuilder {
  ...
  private int mergeFiles(final int cf) throws IOException {
    ...
    for (int i = 0; i < cf; i++) {      
      v[i] = new Values(data, text, i);
      pres[i] = v[i].nextPres();
      t[i] = pres[i].length > 0 ? data.text(Num.read(pres[i], 4), text) : new byte[]{};      
    }
    outl.writeInt(size);
    ...
    while(check(pres)) {
      ...
      if (merge.size() == 0) {
        ...
        t[min] = (pres[min].length > 0) ? data.text(Num.read(pres[min], 4), text) : new byte[]{}; 
      } else {
        TokenBuilder p = new TokenBuilder();
        p.add(new byte[4]);
        int npre = 0;
        int opre = 0;
        for (int j = 0; j < merge.size(); j++) {
          ...
          pres[m] = v[m].nextPres();
          t[m] = pres[m].length > 0 ? data.text(Num.read(pres[m], 4), text) : new byte[]{}; 
        }
  ...
  private boolean check(final byte[][] pres) {
    for (byte[] b : pres) if (b.length > 0) return true;
    return false;
  }
  ...
  /**
   * Writes pre values to disk with number (number of byte values) as first value.
   * @param outl DataOutput
   * @param pres pre values
   * @throws IOException
   */
