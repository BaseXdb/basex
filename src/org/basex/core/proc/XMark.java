 ...
 */
public final class XMark extends AQuery {
  /** Data reference. */
  private Data data;
  /** Root node. */
  private Nodes root;
  ...
  private int xmark6() throws Exception {
    final Result val = qu("count(/site/regions//item)");
    val.serialize(out);
    return 1;
  }
  ...
  private int xmark7() throws Exception {
    final Result val = qu("count(/site//description) + count(/site//mail) " +
      "+ count(/site//email)");
    val.serialize(out);
    return 1;
  }
   ...
   * @return result set
   */
  private XPathProcessor parse(final String xpath) {
    return new XPathProcessor(xpath);
  }
