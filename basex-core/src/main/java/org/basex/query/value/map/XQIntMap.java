...
public final class XQIntMap extends XQHashMap {
  /** Map type. */
  private static final MapType TYPE = MapType.get(BasicType.INTEGER, Types.INTEGER_O);
  /** Hash map. */
  ...
  @Override
  public Itr keyAt(final long index) {
    return Itr.get(map.key((int) index + 1));
  }

  @Override
  public Itr valueAt(final long index) {
    return Itr.get(map.value((int) index + 1));
  }
