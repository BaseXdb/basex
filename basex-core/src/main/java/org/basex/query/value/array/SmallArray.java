...
final class SmallArray extends XQArray {
   ...
   */
  SmallArray(final Value[] members) {
  ...
  public XQArray cons(final Value head) {
    if(members.length < MAX_SMALL) {
      ...
      newMembers[0] = head;
      return new SmallArray(newMembers);
    }
  ...
  public XQArray init() {
    ...
    return new SmallArray(slice(members, 0, members.length - 1));
  }

  @Override
  public XQArray tail() {
    if(members.length == 1) return empty();
    return new SmallArray(slice(members, 1, members.length));
  }
  ...
  public XQArray subArray(final long pos, final long length, final QueryContext qc) {
    ...
    return n == 0 ? XQArray.empty() : new SmallArray(slice(members, p, p + n));
  }
  ...
  XQArray prepend(final SmallArray array) {
    ...
    if(Math.min(a, b) >= MIN_DIGIT) return new BigArray(values, members);
