...
public class QueryException extends Exception {
  ...
  /**
   * Snapshot of a {@link QueryException}, allowing an independent exception to be reconstructed
   * in another thread. The snapshot is expected to be created before the source exception is
   * exposed to other threads. {@link InputInfo} is copied because it lazily initializes line and
   * column data.
   * @param name error QName
   * @param message error message
   * @param info input info (can be {@code null})
   * @param value error value
   * @param error error reference (can be {@code null} for dynamic error messages)
   * @param catchable catchable flag
   * @param markedCol marked error column
   * @param stack XQuery execution stack
   */
  public record Data(QNm name, String message, InputInfo info, Value value, QueryError error,
      boolean catchable, int markedCol, ArrayList<InputInfo> stack) {

    /**
     * Constructor, extracting data from a query exception.
     * @param ex query exception
     */
    public Data(final QueryException ex) {
      this(ex.name, ex.getLocalizedMessage(), ex.info != null ? ex.info.copy() : null,
          ex.value, ex.error, ex.catchable, ex.markedCol, copyStack(ex.stack));
    }

    /**
     * Creates a new query exception from the extracted data.
     * @return query exception
     */
    public QueryException toException() {
      final QueryException ex = new QueryException(info != null ? info.copy() : null, name,
          message);
      ex.error = error;
      ex.value = value != null ? value : Empty.VALUE;
      ex.catchable = catchable;
      ex.markedCol = markedCol;
      for(final InputInfo si : stack) ex.add(si.copy());
      return ex;
    }

    /**
     * Returns copies of the stack entries of a query exception.
     * @param stack stack entries
     * @return copies
     */
    private static ArrayList<InputInfo> copyStack(final ArrayList<InputInfo> stack) {
      final ArrayList<InputInfo> copies = new ArrayList<>();
      for(final InputInfo ii : stack) copies.add(ii.copy());
      return copies;
    }
  }
}
