...
public class CmpG extends Cmp {
  ...
  public final CmpG invert() {
    final Expr expr1 = exprs[0], expr2 = exprs[1];
    final SeqType st1 = expr1.seqType(), st2 = expr2.seqType();
    return st1.one() && !st1.mayBeWrapped() && st2.one() && !st2.mayBeWrapped() ?
  ...
  private Expr optPred(final CompileContext cc) throws QueryException {
    ...
    final Expr expr1 = exprs[0], expr2 = exprs[1];
    if(val.seqType().type instanceof final NodeType nt && nt.kind() != Kind.XNODE &&
      ...
      if(scope != null) {
        ...
        for(final QNm qname : qnames) {
          final Test test = Test.get(nt.kind(), qname, scope, sc().elemNS);
