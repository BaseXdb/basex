 ...
 */
public final class CsvW3MapConverter extends CsvXQueryConverter {
  /**
   * Constructor.
   * @param opts CSV options
   */
  CsvW3MapConverter(final CsvParserOptions opts) {
    super(opts);
  }
  ...
  protected Value finish(final InputInfo ii, final QueryContext qc) throws QueryException {
    ...
    for(final Item col : columns) {
      ++i;
      if(col.string(ii).length > 0 && !columnIndexBuilder.contains(col)) {
