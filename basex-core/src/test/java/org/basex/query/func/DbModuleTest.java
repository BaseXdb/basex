...
public final class DbModuleTest extends AdvancedQueryTest {
  ...
  public void attribute() {
    ...
    query(_DB_ATTRIBUTE.args(NAME, 0, "XXX"), "");
    query(_DB_ATTRIBUTE.args(NAME, "XXX"), "");
  }
  ...
  public void token() {
    ...
    query("data(" + _DB_TOKEN.args(NAME, 0, "XXX") + ")", "");
    query("data(" + _DB_TOKEN.args(NAME, "XXX") + ")", "");
  }
  ...
  public void backups() {
    ...
    query("count(" + _DB_BACKUPS.args(NAME) + "/(@database, @date, @size))", 3);
    query("count(" + _DB_BACKUPS.args(NAME + 'X') + ")", 0);
  ...
  public void replace() {
    ...
    query(_DB_REPLACE.args(NAME, "3.xml", " <a/>") + ',' +
        _DB_REPLACE.args(NAME, "5.xml", " <a/>"));

    // GH-1302: check replacements of new paths
    query(_DB_REPLACE.args(NAME, "6.xml", XML) + ',' + _DB_REPLACE.args(NAME, "7.xml", XML));
    query(_DB_REPLACE.args(NAME, "8.xml", " <a/>") + ',' +
        _DB_REPLACE.args(NAME, "9.xml", " <a/>"));
  }
  ...
  public void export() {
    ...
    query("0[contains(" + _FILE_READ_TEXT.args(f) + ", '&lt;')]", "");
